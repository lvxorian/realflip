...
## Test Stack
Vitest v4 + jsdom + @testing-library/react. **634 tests across 45 files**.
...
- **Potvrzení vyjednané ceny na kartě** (`lead-card.tsx`): prompt používá `type="text"` + `inputMode="numeric"` a tolerantní parser `parseAmountInput` (mezery/NBSP/Kč odfiltruje, jako zbytek aplikace) — `<input type="number">` + `Number()` tiše selhávalo na česky formátovaných cenách („2 500 000" → prázdný value → fajfka nedělala nic). Neplatný vstup ukáže nápovědu. Stejné parsování (`parsePriceInput`) má i pole „Vyjednaná cena" v lead-draweru.
