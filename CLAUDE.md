...
**Edge ochrana = `src/proxy.ts`** (Next 16 náhrada middleware; histórica chyba: `startsWith("/")` veřejná pro vše → mrtvý gate, opraveno Phase 84). Pravidla v `src/lib/proxy-rules.ts` (`isPublicPath` — přesné matchy `/`, `/vop`, prefixy `/login /register /api/auth /report`; `isMachinePath` — cron/action API bez cookie, route si hlídá secret sama). `(dashboard)/layout.tsx` má `auth()` + `redirect` (defense-in-depth).
...
## Mobilní responzivita (Phase 74–77)
- **Breakpoint `lg` (1024px)** odděluje desktop od mobilu. Desktop = sidebar 240/68px (`transition-[width]`, `h-full`), mobil = **bottom nav** (Domů · Nemovitosti · Hledání · Pipeline · Více) + **iOS bottom sheet** (grabber, `rounded-t-3xl`, zvoneček + theme toggle + nav + user footer). Top bar na mobilu neexistuje.
- **Zoom lock**: `viewport` v `layout.tsx` má `maximumScale: 1, userScalable: false` — na mobilu nelze zoomovat (fixní jako nativní app).
- **Globální mobilní CSS** (`globals.css`): iOS zoom fix (`input/select/textarea` 16px ≤640px — jinak Safari autozoom), `main h1` 20px na mobilu, `-webkit-tap-highlight-color: transparent`, `overscroll-behavior-y: contain`.
- `NotificationBell` prop `dropdownAlign` (right v sheetu), dropdown `max-w-[calc(100vw-2rem)]`.
- **Pipeline**: na mobilu (<1024px přes `window.innerWidth` + resize) se místo kanbanu zobrazí **vertikální seznam leadů** (`MobileLeadList`/`MobileLeadRow` v `leads-board.tsx`) — řádky s miniaturou, fází, cenou, skóre, hvězdičkou priority; klepnutí → LeadDrawer. Kanban `hidden lg:block`.
- **Detail nemovitosti**: full-bleed galerie na mobilu (`-mx-3 sm:-mx-4 max-lg:rounded-none lg:rounded-2xl`), spec chips `grid-cols-3`, **kalkulačka sbalitelná** (`InteractiveAnalysis` prop `collapsibleOnMobile` — na mobilu sbalená, hlavička „Kalkulačka · Rozbalit"; analyzátor/desktop vždy rozbalené).
- **Nastavení**: taby = iOS segmented control na mobilu (bílá pilulka), hustší obsah (`space-y-3`, řádky `p-3 sm:p-4`).
- Tap cíle ≥40px napříč aplikací (PctStepper, carousel šipky, favorite, paginace, selecty, close modálů, alerts/tasks). Žádný duplicitní root padding `p-6` (layout dává `p-3 sm:p-4 lg:p-8`).
...
- **Manuální triggers /api/{ares,isir,realingo}/trigger volají logiku IN-PROCESS** (`runAresPoll`/`runIsirPoll`/`syncRealingo`) — auth jen přes session, CRON_SECRET nepotřebují, **žádný self-fetch** na `NEXT_PUBLIC_APP_URL` (self-fetch na špatné APP_URL končil „Sync error: fetch failed"). Sdílené liby: `src/lib/ares/run-poll.ts`, `src/lib/isir/run-poll.ts` (cron + trigger). `NEXT_PUBLIC_APP_URL` se používá už jen pro absolutní e-mailové odkazy (`notify-offers.ts`, rezervační emaily).
...
- **Sloupce**: `flex-1 basis-0 min-w-[150px] lg:min-w-0` — na desktopu se všech 7 vejde bez horizontálního scrollu; @container na sloupci (žádné `@max-[240px]` skrývání obsahu karty).
- **Mobilní seznam** (Phase 77): na mobilu (<1024px přes `window.innerWidth` + resize) se kanban nezobrazuje — místo něj `MobileLeadList`/`MobileLeadRow` (vertikální seznam seskupený podle fází: miniatura, fáze, cena, skóre, Deal/Rezervováno/Propadl krok, hvězdička priority; klepnutí → LeadDrawer). Kanban board `hidden lg:block`.
- **Akce karty na mobilu**: posun/ztraceno/smazat a priorita jsou na mobilu **trvale viditelné** (`max-lg:opacity-100 max-lg:w-auto`), tlačítka `h-9 w-9`.
...
- **Karty investorů jsou celé klikací** (Phase 73): odkaz „Detail" smazán, klik na kartu → `router.push` detail; „Upravit"/„Portál" s `stopPropagation`.
- **Notifikace rezervací** (Phase 72): `GET /api/investors/unread-reservations` → `{ total, byInvestor }` (total = součet byInvestor — konzistence); **`POST`** (body `{ investorId }`) označí nečtené `portal_reservation` notifikace jako přečtené. `MarkReservationsRead` na detailu investora (client komponenta) → otevření investora = přečtení. Dashboard layout refetchuje badge při navigaci. Dropdown zvonečku ukazuje plný text notifikace.
- **Investorem zvolený model** (Phase 71): `leads.portal_reserved_strategy` se propsuje do karty „Portál investorů" na detailu nemovitosti — read-only řádek „Investor zvolil" (`PortalPanel` prop `initialReservedStrategy`).
- **Admin e-maily rezervací** (Phase 71): button „Otevřít investory" → natvrdo `realflip.vercel.app/investors` (logo zůstává z portálu); částka na jednom řádku, bez duplicitního „Kč". Patička e-mailů investorům bez `cakmak@tuta.com`.
- **Investorský portál** (`/investor`, client pages mimo `(dashboard)`): vlastní HMAC cookie session (`src/lib/investor-session.ts`, NEPOUŽÍVÁ NextAuth — investoři nesmí do dashboardu). Přihlášení jménem + heslem (`investors.portal_enabled` / `portal_password_hash` bcrypt). API `/api/investor-portal/login|logout|properties|reserve`. Bílá listina polí dána v `src/lib/investor-portal-view.ts` — UI portálu ani e-mail o NOVÉ nabídce (`offer-template`) neukazují přesnou adresu/GPS/URL/kontakt, jen makrolokalitu `město · čtvrť` (`locationCity`/`locationDistrict`). **Po rezervaci** ale potvrzovací e-mail investorovi záměrně odhaluje plnou adresu + kompletní kontakt na prodávajícího (`properties.contactName/contactPhone/contactEmail` přes `formatPhone`) v `buildReservationEmailHtml({ contact })` — obchod je jeho. Portálový confirmation modal (JSON response `reserve`) zůstává maskovaný na `maskedLocation`. Admin e-maily + in-app notifikace mají plnou adresu vždy. zisk z `leads.stage === "negotiation"` + `portal_visible=1`; „navržená cena" = `stageData.negotiation.currentAmount` fallback `targetPurchasePrice`. Rezervace: `leads.portal_status` (`available`/`reserved`) + `leads.portal_reserved_investor_id` FK investors. **Rezervace drží 72 h** (`PORTAL_RESERVATION_MS`, sloupce `leads.portal_reserved_at` / `portal_expires_at`, nastavované při rezervaci + při admin přiřazení). **Model spolupráce**: `investors.preferred_model` (preference investora) a `leads.portal_reserved_model` (dohodnutý model pro danou nabídku) — hodnoty `flip`/`rent`/`both` dle `COOPERATION_MODELS` v `src/lib/portal-reservation.ts`. Expirace: `expireStaleReservations()` běží při načtení portálu (`listPortalItems`) — prošlá rezervace se uvolní zpět na „Dostupná". Jméno rezervujícího investora se ostatním ukazuje jen jako iniciály (`investorInitials` v `investor-portal-view.ts`, např. „G.S.") — i v e-mailových nabídkách. Admin: panel `src/components/leads/portal-panel.tsx` na detailu nemovitosti, portal sekce v `investor-modal.tsx`, PATCH `/api/leads/[id]/portal`, PATCH/POST `/api/investors*` (bcrypt hash). Detail nemovitosti v portálu (`DealDetail`) zobrazuje detailní položkový rozpad výpočtu z kalkulačky pro flip i nájemní nemovitosti na základě snapshotu z preset API (složky nákladů, poplatky, daně, NOI, cash-on-cash atd.), a neobsahuje řádek "ROI p.a.". Nájemní detail ukazuje „Celková investice" = součet nákladů akvizice + řádky „− Hypotéka (úvěr)"/„LTV"/„Vlastní vklad" (ze snapshotu, capnutý úvěr). Badge typu (FLIP/NAJEM) na kartě = `ModeBadge` — plné pozadí `bg-accent` (zelená) / `bg-info` (modrá) s bílým textem. Migrace `scripts/migrate-portal-reservation.ts` (SQLite + Neon direct non-pooler, `0023_portal_reservation.sql`).
...
- **E-mail loga = rastr, nikdy inline SVG**: Gmail a Seznam.cz vyřezávají inline `<svg>` z HTML e-mailů (tuta.com ho renderuje) → `brickLogoImg(size, baseUrl)` (`src/lib/investor-brick.ts`) vrací `<img src="{baseUrl}/brickon.png">` (bílá značka 319×293, průhledné pozadí, alt „Brickon", `display:block`). Hlavička nabídek: „Právě jsme pro vás vyjednali novou příležitost! Přihlašte se do portálu pro rezervaci a více informací." Gmail externí obrázky blokuje do kliknutí „Zobrazit obrázky" — nevyhnutelné.
