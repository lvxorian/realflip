...
- **Engine robustnost**: váha nabídek dle počtu vzorků (min(sampleSize,8)/8), clamp nabídek do [0,75×; 1,35×] realizovaných, partial pooling (čtvrť nad krajem o >35 % → korekce 0,75/0,25 ke kraji — novostavby), spread 0,05–0,18. DB kompy (Tier 1) vylučují novostavby (condition new) a u „any" aplikují multiplikátory — konzistence se sreality tierem.
