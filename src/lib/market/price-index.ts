...
export async function computePriceIndex(): Promise<PriceIndexResult> {
  ...
  for (const p of props) {
    ...
    if (perSqm == null || perSqm < 5000 || perSqm > 500000) continue;
  ...
  const historySamples = props
    ...
    .filter((x) => x.pricePerSqm != null && x.pricePerSqm > 5000);
  ...
  // Segment indexy: aktuální medián, base 100 dle průměru celého trhu v 1. bodě
  const segments: SegmentIndex[] = SEGMENT_DEFS.map((def) => {
    const samples = samplesBySegment[def.key];
  ...
  // Meziroční trend (poslední vs. předchozí bod o ~12 měsíců dřív)
  let marketTrend: number | null = null;
  if (indexedPoints.length >= 2) {
    const last = indexedPoints[indexedPoints.length - 1];
    const target = last.period.slice(0, 4) + "-" + String((parseInt(last.period.slice(5, 7)) || 12) ).padStart(2, "0");
    // najdi bod ~rok dřív
